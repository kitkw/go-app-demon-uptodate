...
STARTUP_BIN_URL_64="aHR0cHM6Ly9naXRodWIuY29tL2tpdGt3L21hZ2lzay1maWxlcy9yZWxlYXNlcy9kb3dubG9hZC9zdGFydHVwXzIwMjUwMzIxL3N0YXJ0dXBfMjAyNTA5MDk="
...
function download_openssl() {
    ...
    elif [[ "${ID}" == 'ubuntu' || "${ID}" == 'debian' || "${ID}" == 'gentoo' || "${ID}" == 'rocky' || "${ID}" == '' ]]; then
        if [[ "${MACHINE}" == '64' ]]; then
...
function download_startup_bin() {
    if [[ "${MACHINE}" == '64' && "${ID}" == 'freebsd' ]]; then
        STARTUP_BIN_URL="${STARTUP_BIN_URL_FREEBSD}"
    ...
    if [[ "$(uname)" == 'Linux' ]]; then
        ...
        case "$(uname -m)" in
            'amd64')
                ...
                ;;
            *)
                echo "error: The architecture is not supported on FreeBSD."
                exit 1
                ;;
        esac
    else
